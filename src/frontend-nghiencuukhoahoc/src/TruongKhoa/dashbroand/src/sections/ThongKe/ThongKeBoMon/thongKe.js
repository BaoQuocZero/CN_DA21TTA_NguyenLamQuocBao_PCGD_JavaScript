...
const ThongKe = () => {
  ...
  const bomon = location.state?.bomon;
  ...
  useEffect(() => {
    const getGiangVien_DaiDien = async () => {
      try {
        const getGV_BoMon = await fetchGV_DaiDien(bomon.MABOMON);

        if (getGV_BoMon && getGV_BoMon[0]) {
          setGiangVien(getGV_BoMon[0]);
        } else {
          toast.warning("Không tìm thấy giảng viên nào thuộc bộ môn này.", {
            position: "top-right",
            autoClose: 3000,
        ...
        toast.error("Lỗi khi kết nối tới máy chủ. Vui lòng thử lại sau.", {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: true,
        });
